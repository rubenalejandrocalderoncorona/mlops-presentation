...
function Pentagon({ color }: { color: string }) {
  ...
  return (
    <motion.svg viewBox="0 0 100 100" className="w-full h-full" animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: 'linear' }}>
...
function Triangle({ color }: { color: string }) {
  return (
    <motion.svg viewBox="0 0 100 100" className="w-full h-full" animate={{ rotate: -360 }} transition={{ duration: 25, repeat: Infinity, ease: 'linear' }}>
      <polygon points="50,10 90,85 10,85" fill={`${color}20`} stroke={color} strokeWidth="3" />
...
function Diamond({ color }: { color: string }) {
  return (
    <motion.svg viewBox="0 0 100 100" className="w-full h-full" animate={{ rotate: 360 }} transition={{ duration: 30, repeat: Infinity, ease: 'linear' }}>
      <polygon points="50,10 90,50 50,90 10,50" fill={`${color}20`} stroke={color} strokeWidth="3" />
...
export function Slide5_DevOpsToMLOps() {
        ...
        {loops.map((loop, i) => (
          ...
          >
            ...
            <div className="w-44 h-44 relative mb-4">
              ...
              >
                <span className="text-3xl font-black" style={{ color: loop.color, textShadow: `0 0 20px ${loop.color}` }}>
                  {loop.abbr}
                </span>
              </div>
            </div>
            ...
            >
              <h3 className="text-white font-bold text-lg mb-2" style={{ color: loop.color }}>
                {loop.label}
              </h3>
              <p className="text-white/60 text-sm max-w-48 leading-relaxed">{loop.description}</p>
            </motion.div>
          </motion.div>
        ))}
      </div>
